...
### 実装完了後は即PR作成
実装完了後は「コミットしますか？」等の確認を挟まず、作業ブランチへのコミット → push → PR作成まで一気に進めること。ユーザーへの確認は不要。
...
**`develop`・`main` への直接 push は禁止**。変更は必ず作業ブランチから PR 経由で取り込むこと。

### コードレビュー必須
実装完了後、必ずコードレビューを受けること。作業ブランチを push して PR を作成したら、`/review <PR番号>` スキル（Claude Code 標準）でレビューし、指摘があれば修正して追加 push する。
